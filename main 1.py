...
def _organic_text(results: list[dict]) -> str:
    """
    Flatten Bright Data SERP results into plain text for the LLM.

    Bright Data's Google SERP dataset (gd_mfz5x93lmsjjjylob) returns one
    document per *page*, where organic results live inside:
        result[0]["organic"] = [{"title": ..., "description": ..., "url": ...}, ...]

    This function handles BOTH that nested layout AND a flat list of result
    dicts (fallback / mock), so it works in all environments.
    """
    flat: list[dict] = []

    for r in results:
        # ── Nested layout: {"organic": [...], "paid": [...], ...}
        if "organic" in r and isinstance(r["organic"], list):
            flat.extend(r["organic"])
        # ── Also pull "general" results block if present
        if "general" in r and isinstance(r["general"], list):
            flat.extend(r["general"])
        # ── Flat layout (mock / older schema): the dict itself is a result
        elif "title" in r or "description" in r or "snippet" in r:
            flat.append(r)

    lines = []
    for item in flat[:12]:                        # cap at 12 organic hits
    ...
    if not lines:
        # Last-resort: dump raw JSON so the LLM at least sees something
        raw = json.dumps(results[:3], ensure_ascii=False)[:1200]
...
class CompanyWorkerState(TypedDict):
    ...
    # Allows dashboard_output_node to write into the global State.final_gtm_reports
    # reducer (operator.add). Without this key LangGraph silently drops the value.
    ...
    return [
        Send(
            "company_researcher",
            {
                ...
                "final_output": {},
                "final_gtm_reports": [],   # required by CompanyWorkerState
            },
        )
        for c in accounts
    ]
...
def score_router_node(state: CompanyWorkerState) -> str:
    ...
    Routes based on intent score:
      == 0  → retry_planner   (truly zero signal — try broader queries)
      1-59  → competitor_context  (warm lead, enrich with competitive intel)
      ≥ 60  → deep_research   (hot lead, do full research before pitch)

    NOTE: threshold was deliberately kept low (0 = retry, not < 20) because
    Bright Data SERP returns real data and even modest signals (score 5-15)
    deserve the competitor_context enrichment path rather than a retry loop.
    """
...
def accumulate_reports(state: State) -> Dict[str, Any]:
    ...
    # UNKNOWN comes from retry_planner path — treat as LOW for display
